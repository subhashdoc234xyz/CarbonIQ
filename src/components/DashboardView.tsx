...
interface DashboardViewProps { logs: ActivityLogEntry[]; onNavigateToLogs: () => void; onNavigateToOptimizer: () => void; }

export const DashboardView: React.FC<DashboardViewProps> = ({ logs, onNavigateToLogs, onNavigateToOptimizer }) => {
  ...
  return <div className="w-full max-w-5xl mx-auto space-y-6 pb-8 animate-fade-in">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><Metric label="Total emissions" value={`${(totalKg / 1000).toFixed(2)} tCO₂e`} /><Metric label="Activity records" value={String(logs.length)} /><Metric label="Reduction projects" value="0" /></div>
