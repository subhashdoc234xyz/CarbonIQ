...
export const ActivityLogView: React.FC<ActivityLogViewProps> = ({ logs, onAddLog, onDeleteLog }) => {
  ...
  const [factor, setFactor] = useState(String(DEFAULT_EMISSION_FACTORS[0].defaultFactor));
  ...
  return <div className="w-full max-w-4xl mx-auto space-y-6 pb-12 animate-fade-in">
    ...
    <form onSubmit={submit} className="rounded-2xl bg-[#16181C] border border-[#26292F] p-5 sm:p-6 space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Activity type"><select value={activityType} onChange={(e) => { const next = DEFAULT_EMISSION_FACTORS.find((item) => item.value === e.target.value)!; setActivityType(next.value); setFactor(next.defaultFactor === undefined ? '' : String(next.defaultFactor)); }} className="input">{DEFAULT_EMISSION_FACTORS.map((item) => <option key={item.value} value={item.value}>{item.label} ({item.unit})</option>)}</select></Field>
      ...
      <div className="rounded-xl bg-[#0B0D10] border border-[#26292F] p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between"><div><p className="text-xs text-[#9CA3AF]">Calculated emissions</p><p className="text-2xl font-bold font-mono text-[#34D399]">{emissions.toLocaleString(undefined, { maximumFractionDigits: 3 })} kg CO₂e</p></div>{selected.sourceUrl ? <div className="max-w-sm text-xs text-[#9CA3AF]"><p>Default: <a className="text-[#34D399] hover:underline" href={selected.sourceUrl} target="_blank" rel="noreferrer">{selected.sourceName}</a></p><p className="mt-1">{selected.note} You may edit the factor.</p></div> : <p className="max-w-sm text-xs text-[#9CA3AF]">Enter a verified factor for this custom activity.</p>}</div>
